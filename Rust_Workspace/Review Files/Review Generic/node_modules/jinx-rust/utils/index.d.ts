...
declare type NodeValues<T> = T extends FunctionDeclarationParameters ? T[number] | T["self"] : T extends ReadonlyArray<any> ? T[number] : T;
...
/** Iterate childNodes */
declare function each_childNode(node: Node, callback: AstItEachFn): void;
/** Iterate nodes in tree, bottom up, excluding itself */
declare function each_node(ast: Node, callback: AstItEachFn): void;
...
declare function reassignNodeProperty(value: any, parent: Node, key: string, index?: number | "self" | undefined): void;
...
declare function is_Index(node: Node): node is Index;
...
declare function is_MacroRule(node: Node): node is MacroRuleDeclaration | MacroInlineRuleDeclaration;
...
declare function is_LiteralRawStringLike(node: Node): node is Literal & { kind: LiteralKind.rString | LiteralKind.brString };
