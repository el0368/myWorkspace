...
var doc = require('prettier/doc.js');
...
function isIdent(node, name) {
  return !!node && utils.is_Identifier(node) && node.name === name;
}
function isToken(node, tk) {
  return !!node && utils.isTK(node, tk);
}
function isGroup(node, dk) {
  return !!node && utils.is_DelimGroup(node) && node.segments.dk === dk;
}
...
function insert_blocks(node, parent, key, index) {
  if (parent && key) {
    if (!utils.is_ExpressionStatement(parent) && (utils.is_FlowControlExpression(node) || !isReadingSnippet() && utils.is_ReassignmentNode(node) && !(utils.is_ReassignmentNode(parent) && parent.left === node))) {
      utils.reassignNodeProperty(blockify(node), parent, key, index);
    } else if (utils.is_ClosureFunctionExpression(node) && (!!node.returnType && !utils.is_BlockExpression(node.expression) || utils.is_ExpressionWithBodyOrCases(node.expression) && !utils.is_BlockExpression(node.expression) && !utils.is_IfBlockExpression(node.expression))) {
...
function needsOuterSoftbreakParens(node) {
  ...
  if (utils.is_FlowControlMaybeValueExpression(parent) && parent.expression === node && flowControlExpressionNeedsOuterParens(parent)) {
    return true;
  }
...
function canInlineBlockBody(node) {
  ...
  if (body.length === 1) {
    ...
    if (utils.is_ExpressionStatement(stmt) && !needsSemi(node, stmt)) {
      ...
      if (utils.is_FlowControlExpression(expr) || utils.is_ClosureFunctionExpression(expr) || utils.is_ExpressionWithBodyOrCases_or_BlockLikeMacroInvocation(expr)) {
        return false;
      }
      return canInlineInlineable(node);
    }
...
function canInlineInlineable(node) {
  ...
  if (utils.is_IfBlockExpression(node)) {
    if (!node.else || utils.is_ExpressionWithBodyOrCases_or_BlockLikeMacroInvocation(node.condition) || willBreak(getPrintFn()("condition"))) {
      return false;
    }
...
function printRuleTransform(print4, node, t = utils.getDelimChars(node.transform)) {
  ...
  if (/^. *\n/.test(text)) {
    ...
    if (utils.is_DelimGroup(segment) || utils.is_MacroGroup(segment)) {
      const inner = utils.is_DelimGroup(segment) ? utils.getDelimChars(segment.segments) : { left: "$(", right: `)${((_a = segment.sep) == null ? void 0 : _a.loc.getOwnText()) ?? ""}${segment.kind}` };
      return [
        dedentToRoot([
          t.left,
          [
...
function isComplexDestructuring(node) {
  if (utils.is_ReassignmentExpression(node)) {
    ...
    return utils.is_StructLiteral(leftNode) && leftNode.properties.length > 2 && leftNode.properties.some((property) => utils.is_StructLiteralProperty(property) || utils.is_StructLiteralPropertySpread(property));
  }
  if (utils.is_VariableDeclarationNode(node) || utils.is_MatchExpressionCase(node) || utils.is_LetScrutinee(node)) {
    ...
    return utils.is_StructPattern(leftNode) && leftNode.properties.length > 2 && leftNode.properties.some((property) => utils.is_StructPatternPropertyDestructured(property));
  }
  return false;
}
...
function printBlockBody(print4, node) {
  ...
  return [
    "{",
    body.length > 0 ? ((_a = utils.getBodyOrCases(node)) == null ? void 0 : _a.length) ? canInlineBlockBody(node) ? [indent([line, body]), line] : group([indent([line, body]), line], { shouldBreak: true }) : body : emptyContent(node),
    "}"
  ];
}
...
function adjustClause(node, doc2) {
  return [
    "whereBounds" in node && (!!node.whereBounds || utils.hasTypeBounds(node) && node.typeBounds.length > 1) && willBreak(doc2) ? ifBreak(hardline, " ", { groupId: getHeritageGroupId(node) }) : " ",
    doc2
  ];
}
...
function printBinaryishExpression(print4, node) {
  ...
  if (isInsideParenthesis)
    return parts;
  if (is_CallExpression_or_CallLikeMacroInvocation(parent) && parent.callee === node || utils.is_UnaryExpression(parent) || utils.is_MemberExpression(parent)) {
...
function printArrayLike(print4, node) {
  ...
  const shouldBreak = !utils.is_TupleNode(node) && node.items.length > 1 && node.items.every((item, i) => {
...
function handleEndOfLineComment(ctx2) {
  ...
  if (precedingNode && enclosingNode && is_CallExpression_or_CallLikeMacroInvocation(enclosingNode) && enclosingNode.arguments.length > 0 && precedingNode === (enclosingNode.typeArguments ? last_of(enclosingNode.typeArguments) : enclosingNode.callee)) {
    ...
    return true;
  } else if (enclosingNode && utils.is_StructLiteralProperty(enclosingNode)) {
...
function handleFunctionComments(ctx2) {
  ...
  if (enclosingNode && utils.is_FunctionNode(enclosingNode)) {
    ...
    } else if (followingNode && isStartOfLine(comment) && comment.loc.isAfter(enclosingNode.parameters) && (!utils.is_FunctionDeclaration(enclosingNode) || !enclosingNode.whereBounds || comment.loc.isAfter(enclosingNode.whereBounds)) && (!enclosingNode.returnType || comment.loc.isAfter(enclosingNode.returnType)) && followingNode === (utils.is_FunctionDeclaration(enclosingNode) ? (_a = enclosingNode.body) == null ? void 0 : _a[0] : enclosingNode.expression)) {
      addLeadingComment(followingNode, comment);
    }
  }
}
...
function print_comment(comment) {
  const doc2 = utils.is_BlockCommentNode(comment) ? isIndentableBlockComment(comment.value) ? [
...
function canAttachComment(n) {
  return !utils.is_Comment(n) && !isNoopExpressionStatement(n) && !utils.is_MissingNode(n) && !utils.is_PunctuationToken(n);
}
var plugin = {
  languages: [
    {
